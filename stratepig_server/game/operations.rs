...
impl GameServer {
    pub async fn run_operations(&self, room: &GameRoom, is_placement: bool) {
        ...
        let opp_player = self.get_client(room.other_id(*id)).unwrap().player.as_ref().unwrap();
        ...
        let mut local_success = false; let mut enemy_success = false;
        ...
        'outer: for i in 1..3 {
            ...
            for piece in board.into_iter() {
                ...
                surrounding = surrounding.into_iter().filter(|x| *x > 0 && *x < 100).collect();
                ...
                for tile in surrounding {
                    ...
                    if total_board.iter().any(|x| x.location == tile) {
                        if i == 1 {
                            if enemy_board.iter().any(|x| x.location == tile) && !local_board.iter().any(|x| x.location == tile) {
                                local_success = true;
                                continue 'outer;
                            } else {
                                continue;
                            ...
                            if local_board.iter().any(|x| x.location == tile) && !enemy_board.iter().any(|x| x.location == tile) {
                                enemy_success = true;
                                continue 'outer;
                            } else {
                                continue;
        ...
        if !(local_success && enemy_success) {
            ...
            if !local_success &&  enemy_success {
                self.broadcast_win_i(&room, PlayerRole::Two, WinType::OutOfMoves, is_placement).await;
            } else if !enemy_success && local_success {
                self.broadcast_win_i(&room, PlayerRole::One, WinType::OutOfMoves, is_placement).await;
            } else {
                self.broadcast_win_i(&room, PlayerRole::Tie, WinType::OutOfMoves, is_placement).await;
            }
        }
    }
}